...
/** better-auth user (email/password + future OAuth) */
...
/** Credential / OAuth provider accounts (password lives here for email provider) */
...
/** Per-user read state for feed timeline articles (not the later-reading list). */
export const articleStates = sqliteTable(
  ...
  (table) => [uniqueIndex('article_states_user_article_idx').on(table.userId, table.articleId)],
)
